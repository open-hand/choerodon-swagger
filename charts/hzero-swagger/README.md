...
```bash
$ helm install c7n/hzero-swagger --name hzero-swagger
```
...
`image.repository` | 镜像库地址 | `registry.choerodon.com.cn/choerodon/hzero-swagger`
...
`service.name` | service名称 | `hzero-swagger`
...
```bash
$ helm install c7n/hzero-swagger \
    --set env.open.SKYWALKING_OPTS="-javaagent:/agent/skywalking-agent.jar -Dskywalking.agent.application_code=hzero-swagger  -Dskywalking.agent.sample_n_per_3_secs=-1 -Dskywalking.collector.backend_service=oap.skywalking:11800" \
    --name hzero-swagger
```

## 验证部署
```bash
curl -s $(kubectl get po -n c7n-system -l choerodon.io/release=hzero-swagger -o jsonpath="{.items[0].status.podIP}"):8261/actuator/health | jq -r .status
```
出现以下类似信息即为成功部署
